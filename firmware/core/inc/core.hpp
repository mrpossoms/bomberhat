...
template <size_t                             X_SIZE,
          size_t                             Z_SIZE,
          size_t                             U_SIZE,
          IEstimator<X_SIZE, Z_SIZE, U_SIZE> EST,
          IController<X_SIZE, U_SIZE>        CON,
          typename... SENSORS>
struct Core
{
	Core() { add_sensors<0, SENSORS...>(); }

	void step(bh::Context& ctx)
	{
		// for each polling device, poll. for any event driven sensors capture
		// queued observations. aggregate into measurement vector
		for (auto& s : sensors)
		{
